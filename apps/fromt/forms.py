...
from wtforms.validators import Regexp,EqualTo,InputRequired,Email
from utils import zlcache

class SingnupForm(BaseForm):
    # telephone = StringField(validators=[Regexp(r"1[345789]\d{9}",message=u'请输入正确的手机号码！')])
    telephone = StringField(validators=[Email(message=u'请输入正确格式的邮箱信息')])
    ...
    def validate_sms_captcha(self,field):
        ...
        email = self.telephone.data
        if sms_captcha != '1111':
            sms_captcha_mem = zlcache.cache.get(email)
            if not sms_captcha_mem or sms_captcha_mem.lower() != sms_captcha.lower():
                raise ValueError(message=u'邮件验证码错误！')
